...
## 文字列のフォーマットの整え方（sprintf）
```
<?php
  $date = sprintf('%04d.%02d.%02d', 2021, 1, 1); //d = デシマル（整数）・s = 文字
  echo $date;
?>
```
`2021.01.01`と出力される。（前側にフォーマットを指定, その後ろにフォーマットの数だけパラメーターを記述する）
...
## 繰り返し処理
### while構文
繰り返し処理の１種類である。（）の中に条件を設けて`while`から`endwhile`の間で実行される処理の内容を記述する。複雑な条件のとき（初期化の処理と繰り返しの処理がずれているときや何回繰り返すかわからない時）
...
```
### for構文
繰り返し処理の１種類である。（変数初期化;繰り返し条件;更新処理）に条件を設けて`for`から`endfor`の間で実行される処理の内容を記述する
